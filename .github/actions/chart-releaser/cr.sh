...
Usage: $(basename "$0") <options>
    ...
    -x, --helm-dir         The charts directory (default: charts)
        ...
        case "${1:-}" in
            ...
            -x|--helm-dir)
                if [[ -n "${2:-}" ]]; then
                    ...
                    echo "ERROR: '-x|--helm-dir' cannot be empty." >&2
                    show_help
                    exit 1
                fi
                ;;
...
update_index() {
    ...
    python $GITHUB_ACTION_PATH/convert_chart_index.py ./$helm_dir/index.yaml
    ...
    git add .
    git commit -am "Chart release for $(git remote get-url origin) "
    git push
    cd $p_pwd
}
